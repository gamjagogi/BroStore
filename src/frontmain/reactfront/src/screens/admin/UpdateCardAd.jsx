...
const UpdateCardAd = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const response = await axios.get(`/admin/card/${userId}`, {
                headers: {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (accessToken && refreshToken) {
                // 요청 보내기
                const response = await axios.post(`/admin/card/save/${userId}`, JSON.stringify(requestData), {
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${accessToken}`,
                        'RefreshToken': `Bearer ${refreshToken}`,
    ...
    const onDeleteAd = async (adId) => {
        ...
        const userId = sessionStorage.getItem('userData2');
        try {
            const response = await axios.post(`/admin/card/delete/${userId}`, adId, {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${accessToken}`,
                    'RefreshToken': `Bearer ${refreshToken}`,
        ...
        <React.Fragment>
            ...
            >
                <div className="container text-center">
                    <span className="display-5 px-3 bg-white rounded shadow">AD Card Config</span>
                </div>
            </div>
            <br/>
                        ...
                        <Card>
                            ...
                            <Card.Body>
                                ...
                                <Form.Group className="mb-3" controlId="formBasicText">
                                    ...
                                    <Form.Control type="text" placeholder="Enter title" maxLength={60}
                                                  onChange={(e) => changeTitle(e)}/>
                                </Form.Group>
                                <Form.Group className="mb-3" controlId="formBasicText">
                                    <Form.Label>Product description</Form.Label>
                                    <Form.Control type="text" placeholder="Enter description" maxLength={60}
                                                  onChange={(e) => changeDescription(e)}/>
                                </Form.Group>
                                <Form.Group className="mb-3" controlId="formBasicUrl">
                                    <Form.Label>Product url</Form.Label>
                                    <Form.Control type="text" placeholder="Enter Product url" maxLength={60}
